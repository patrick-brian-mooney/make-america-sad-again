...
<meta name="date" content="2017-01-18T15:55:05-0800" />
...
if debugging:
    import pprint
    print("<h4>DEBUGGING: Submitted data was</h4>")
    pprint.pprint('<p><strong>%s</strong></p>' % form)
...
if 'tweet_text' in form:     # Then we're evaluating a previous quiz, and should provide feedback.
    ...
    if form['who'].value.strip('@') == form['orig_account'].value.strip('@'):   # User guess was correct
        if form['who'].value.strip('@') == tu.target_twitter_id.strip('@'):     # User correctly guessed it was The Donald
        ...
        if form['who'].value.strip('@') == tu.target_twitter_id.strip('@'):     # User incorrectly guessed it was The Donald
